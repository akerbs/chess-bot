...
export class TelegramBotAdapter implements BotPlatform {
  ...
  onWebAppData(handler: (ctx: BotContext, data: string) => Promise<void> | void): void {
    this.bot.on('web_app_data', async (ctx) => {
      if (!ctx.from || !('data' in ctx.webAppData)) return;
      
      const botCtx = this.toBotContextFromWebApp(ctx);
      const data = ctx.webAppData.data as string;
      
      try {
        await handler(botCtx, data);
      } catch (error) {
        logger.error(error, 'Ошибка в обработчике web app data');
      }
    });
  }
  ...
  private toBotContextFromWebApp(ctx: Context): BotContext {
    if (!ctx.from) {
      throw new Error('Context without user');
    }

    const chatId = ctx.chat?.id || (ctx.message && 'chat' in ctx.message ? ctx.message.chat.id : undefined);
    if (!chatId) {
      throw new Error('Invalid web app context');
    }

    const messageId = ctx.message && 'message_id' in ctx.message ? ctx.message.message_id : 0;
